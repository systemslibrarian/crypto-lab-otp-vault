 ...
 *    oklab, var(--accent) 6%, transparent)` wash and every ciphertext byte's
 *    border is a `color-mix()` of the accent, so axe files those under
 *    `incomplete` rather than measuring them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab fades the hero
 *    subtitle to `.85`, every byte's index caption to `.7`, every unknown byte
 *    in a reconstruction (its `··` hex and `?` glyph) to `.5`, every
 *    non-printable revealed cell to `.8`, and disabled ghost buttons to `.4`.
 *    Note this lab collapses animation and transition durations to 0.001ms
 *    under reduced motion and declares no keyframes at all, so nothing here can
 *    leave a scan waiting on a loop that never ends.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This is the
 *    central case here: every `.strip` is a non-wrapping row of hex-over-glyph
 *    byte columns inside an `overflow-x: auto` wrapper, and a 40-byte message
 *    puts most of its bytes outside that box at any moment. Content scrolled
 ...
 *    fall through to white, inventing a failure for hundreds of cells at a time.
 *    Skip it, and rely on the bytes that ARE in view, which are measured for
 *    real — the crib chip scrolls its own window into view, so the interesting
 *    ones always are.
 *
 ...
 *    draws every strip, chip and reveal cell out of plain divs and spans, and
 *    its only SVG is the shared header's `aria-hidden` hamburger icon — so this
 *    rule finds nothing here today. It is kept because it is cheap and silent
 *    when unused: an `<svg>`
 *    draws a shape as a `<rect>` and then its label as a following `<text>`, so
 *    a label's backdrop is a PRECEDING SIBLING shape, not an ancestor. An
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" card and every ciphertext byte's border are authored with
     * `color-mix()`, and Chromium reports that
     ...
     * wash fading to `transparent` invents backdrops for text a full page below
     * it (accent links read 3.56:1 elsewhere in this fleet where they actually
     * render ~4.9:1). So each gradient is *sampled* at the text's real location
     ...
     * (`url()`, `none`) is unmeasurable and paints nothing. This lab uses no
     * gradients today; the branch is kept because it is silent when unused and
     * the alternative is a helper that mismeasures the first one added.
     */
     ...
     *
     * A driven scan here walks a dozen byte strips of up to forty three-span
     * columns each — well over a thousand text-owning nodes — and every one of
     * them re-walks the same handful of ancestors up to `<body>`. Without the
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has no <details>; what it has is the `hidden`
      // attribute swapping the walkthrough's start button for its panel, which
      // `checkVisibility()` catches for the same reason. The gate scans both
      // halves of that toggle rather than un-hiding either.
     ...
     * it, the `aria-hidden` decoration this lab carries — each panel's emoji
     * icon, each status line's ✓ / ⛔ / ⚠ marker and the intro legend's three
     * swatches — would be measured as if each took its element's declared
     * `color`, reporting ratios for marks that carry no text at all.
     */
     ...
     *
     * This lab builds its DOM with createElement rather than markup strings, so
     * the rule is inert here — but it is the kind of mistake that is cheap to
     * guard: interpolating a string ARRAY into
